...
class WebSocketClient:
    ...
    def connect(self, task_id: Optional[str] = None):
        if not _HAS_WEBSOCKETS:
            print("websockets 未安装，跳过 WebSocket 连接")
            return
        if self._running:
            self.disconnect()
...
def process_incoming_messages() -> bool:
    ...
    for msg in new_msgs:
        ...
        if msg_type == "progress":
            ...
            statuses = st.session_state.get("ai_statuses", {})
            statuses[msg.get("role", "?")] = msg.get("status", "idle")
            st.session_state.ai_statuses = statuses
        elif msg_type == "llm_retry":
            statuses = st.session_state.get("ai_statuses", {})
            statuses["llm"] = "retrying"
            st.session_state.ai_statuses = statuses
            # 同时加到消息列表
            messages_to_add.append({
                "sender_role": "system",
                "content": f"⚠️ {msg.get('message', '')}",
...
def get_retry_status() -> dict:
    """获取最近的 LLM 重试状态（只查看不消费消息）"""
    ws_client = st.session_state.get("ws_client")
    if not ws_client:
        return {}
    # 使用内部队列的队列机制 peek 消息，不 drain
    q = ws_client._message_queue
    with q.mutex:
        items = list(q.queue)
    for msg in items:
        if msg.get("type") == "llm_retry":
            return msg
    return {}
