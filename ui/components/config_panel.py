...
def render_config_panel():
    ...
    if api is None:
        st.warning("API 客户端未初始化，请在设置中配置 API 地址")
        return
    ...
    # Normalize: ensure each instance has 'id' (database returns instance_id)
    for i in instances:
        if 'instance_id' in i and 'id' not in i:
            i['id'] = i['instance_id']
    ...
    if st.session_state.get("show_instance_form"):
        with st.form("ai_instance_form"):
            ...
            inst_id = st.text_input("ID", value=edit.get("id", "") if is_edit else "", disabled=is_edit)
            name = st.text_input("名称", value=edit.get("name", "") if is_edit else "")
            endpoint = st.text_input("API 端点", value=edit.get("endpoint", "") if is_edit else "http://localhost:11434")
            model = st.text_input("模型名", value=edit.get("model", "") if is_edit else "qwen2.5:7b")
            api_key = st.text_input("API Key", type="password", value=edit.get("api_key","") if is_edit else "")
            tags_str = st.text_input("标签 (逗号分隔)", value=",".join(edit.get("tags", [])) if is_edit else "")
    ...
    with tabs[0]:
        ...
        with col1:
            new_max_retries = st.number_input("最大重试次数 (0=无限重试)", 
                value=int(ai_retry.get("max_retries", 10)), min_value=0, step=1)
            ...
            new_call_timeout = st.number_input("调用超时(秒) (0=无限等待)",
                value=int(ai_retry.get("call_timeout", 600)), min_value=0, step=1)
        with col2:
        ...
        if st.button("保存 AI 重试设置", use_container_width=True):
            updates = {
                ...
                "ai_pool.retry.call_timeout": new_call_timeout,
