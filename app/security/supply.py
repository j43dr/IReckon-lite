...
class SupplyChainFirewall:
    def __init__(self):
        self._pip_blacklist = ["malicious-package", "pycrypto-demo", "secrethash", "thisisafakedpy"]
        ...
        if "pip" in parts and "install" in parts:
            for word in parts:
                ...
                if pkg in self._pip_blacklist:
                    logger.warning(f"Supply chain blocked pip: {pkg}")
                    return False
            ...
            for word in parts:
                ...
                if pkg in self._npm_blacklist:
                    logger.warning(f"Supply chain blocked npm: {pkg}")
                    return False
        return True
