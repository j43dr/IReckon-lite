...
class ExecuteCodeTool(BaseTool):
    ...
    def __init__(self):
        super().__init__("execute_code", "执行Python代码（支持沙箱/本地模式）", ToolType.EXECUTE_CODE)
    
    async def execute(self, code: str, timeout: int = 30, **kwargs) -> ToolResult:
        error = self.validate_params({"code": code}, ["code"])
        if error:
            return ToolResult(False, error=error)
        from app.security.filter import security_filter
        check = await security_filter.check_code(code)
        if not check["allowed"]:
            return ToolResult(False, error=f"安全拦截: {check['reason']}")
        try:
            from app.tools._execute_code_impl import execute_safe
            result = await execute_safe(code, timeout)
...
class ShellTool(BaseTool):
    ...
    def __init__(self):
        super().__init__("shell", "执行Shell命令（受安全策略保护）", ToolType.SHELL)
    ...
    async def execute(self, command: str, timeout: int = 30, **kwargs) -> ToolResult:
        ...
        if error:
            return ToolResult(False, error=error)
        from app.security.filter import security_filter
        check = await security_filter.check_command(command)
        if not check["allowed"]:
            return ToolResult(False, error=f"安全拦截: {check['reason']}")
        from app.security.mining import MiningDetector
        if MiningDetector().scan_command_line(command):
            return ToolResult(False, error="安全拦截: 检测到挖矿行为")
        try:
