...
class PartsLibrary:
        ...
        await db.execute(
            """INSERT INTO tool_parts
               (part_id, name, description, language, code,
                input_schema, output_schema, tags, model_path, created_by)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            (
                part_id,
                name,
    ...
    async def search(
        ...
        for row in rows:
            parts.append({
                ...
                "code": row.get("code", ""),
                "input_schema": json.loads(row["input_schema"]) if row.get("input_schema") else {},
    ...
    async def get_part(self, part_id: str) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            "model_path": row[8] if len(row) > 8 else "",
