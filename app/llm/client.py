...
def get_model_max_tokens(model_name: str) -> Dict[str, int]:
...
class LLMClient:
    ...
    async def _try_call(self, cap, messages, temp, max_tok, cancel_evt, max_retries, infinite_retry, **kwargs):
        ...
        effective_infinite = infinite_retry or retry_cfg.get("infinite_retry", False)
    ...
    async def _call_non_stream(self, cap, messages, temp, max_tok, cancel_evt, max_retries, infinite_retry, fallback_caps=None, **kwargs):
        ...
        async with sem:
            ...
            max_outer_retries = 50
            outer_attempt = 0
            while outer_attempt < max_outer_retries:
                outer_attempt += 1
                if cancel_evt and cancel_evt.is_set():
                    raise LLMCallError("用户取消")
            ...
            raise LLMCallError(f"主备模型均失败，已达最大重试次数({max_outer_retries})")
